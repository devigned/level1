...
if [[ -z "$(az ad sp show --id http://${sp_name})" ]]; then
    ...
    service_principal=$(az ad sp create-for-rbac -n ${sp_name} --cert "$(az keyvault certificate show --vault-name ${existing_name} -n ${cert_name} --query "cer" -o tsv)")
    echo ${service_principal}
else
    echo "Using the following service principal:"
...
if [[ "$(az vm show -g ${group_id} -n ${vm_name} --query "provisioningState=='Success'")" == 'false' ]]; then
    ...
    az vm create -g ${group_id} -n ${vm_name} --admin-username deploy --image debian --secrets "$vm_secret" --custom-data "${DIR}/cloud-config.yml" 1>/dev/null
